 ...
 * Provides a tactile representation of the Indian voting process.
 */
export function VVPATSimulator() {
   ...
   * @param {Object} candidate - The candidate object selected.
   */
  function handleVote(candidate) {
    if (step !== 'ready') return;
    setSelected(candidate);
    setStep('voting');
    trackEvent('simulation_vote_cast', { candidate: candidate.name, party: candidate.party });

    setTimeout(() => {
      setStep('printing');
      setVvpatVisible(true);
      trackEvent('simulation_printing');

      setTimeout(() => {
        setVvpatVisible(false);
        setStep('success');
        trackEvent('simulation_success');
      }, VVPAT_VISIBLE_MS);
  ...
  return (
    <section className="premium-card overflow-hidden" aria-labelledby="vvpat-title">
      <div className="border-b border-civic-line bg-slate-50 p-5">
        <h2 id="vvpat-title" className="text-xl font-extrabold text-ink">EVM & VVPAT Simulation</h2>
      ...
      <div className="grid gap-6 p-6 lg:grid-cols-2">
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-b-lg border-x border-b border-slate-300 bg-slate-100 p-4 shadow-inner">
            ...
            {step === 'voting' && (
              <div className="mt-4 flex items-center justify-center gap-2 rounded bg-orange-50 p-2 text-xs font-bold text-eci-saffron animate-pulse" role="alert">
          ...
          <div className="w-full max-w-[280px] space-y-4">
            <div className="relative rounded-xl border-4 border-slate-700 bg-slate-800 p-4 shadow-xl">
              ...
              <div className="relative h-64 w-full overflow-hidden rounded bg-slate-900 shadow-inner ring-4 ring-slate-700" role="status" aria-live="polite">
                <div className="absolute inset-0 bg-gradient-to-b from-black/20 to-transparent pointer-events-none z-10" aria-hidden="true" />
              ...
              <div className="absolute inset-x-4 top-8 h-64 pointer-events-none bg-gradient-to-tr from-white/10 to-transparent z-20 opacity-30" aria-hidden="true" />
            </div>
            ...
            <div className="rounded-lg bg-orange-50 p-4 text-center text-xs text-orange-900 border border-orange-100">
              ...
              {step === 'printing' && (
                <div className="font-bold text-[#9a5a00]" role="alert">
                  Verify your selection on the paper slip. It will be visible for 7 seconds.
                </div>
              )}
              {step === 'success' && (
                <div className="space-y-3">
                  <div className="flex items-center gap-2 justify-center font-bold text-[#138808]" role="status">
                  ...
                  <button
                    type="button"
                    onClick={reset}
                    aria-label="Reset VVPAT simulator to try again"
                    className="inline-flex items-center gap-1 font-bold text-[#9a5a00] hover:underline transition-all"
                  >
